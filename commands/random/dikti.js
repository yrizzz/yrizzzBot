...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        try {
            ...
            const result = await req('GET', `https://yrizzz.my.id/api/v1/random/dikti?data=${data}`);
            if (result.status) {
                await ctx.reply({ text: JSON.stringify(result.data, null, 2) }, { ephemeralExpiration: m?.message?.extendedTextMessage?.contextInfo?.expiration ?? 0 });
                await ctx.react(ctx.id, '✅');
            }
